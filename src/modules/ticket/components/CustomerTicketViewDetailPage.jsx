...
export default function CustomerTicketViewDetailPage() {
  ...
  const { mutateAsync: submitComment, isPending: isSubmittingComment } = useMutation({
  ...
  return (
    <div style={{
      ...
      <div style={{ background: "white", padding: "15px 30px", boxShadow: "0 2px 5px rgba(0,0,0,0.05)", height: "70px" }} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "24px", alignItems: "start" }}>
                  ...
                  <div style={{ padding: "12px 6px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px" }}>
                      ...
                      <div>
                        <div style={{ color: "#FF8040", fontWeight: "600", fontSize: "13px", marginBottom: "4px" }}>{t("pages.ticketDetail.handler")}</div>
                        <div style={{ color: "#333" }}>{ticket.handler || "-"}</div>
                      </div>
                      <div>
                    ...
                    <div style={{ marginTop: "26px", paddingTop: "20px", borderTop: "1px solid #f1f5f9" }}>
                      ...
                      <form onSubmit={handleSubmitResponse}>
                        <textarea
                          ...
                          style={{ width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #ddd", resize: "none", fontSize: "14px", outline: "none", marginBottom: "12px", boxSizing: "border-box" }}
                        />
                        <div style={{ display: "flex", justifyContent: "flex-end" }}>
                          <button
                            type="submit"
                            disabled={isSubmittingComment || !responseText.trim()}
                            style={{
                              padding: "10px 30px", borderRadius: "8px", border: "none",
                              background: "#FF8040", color: "white", fontWeight: "600",
                              cursor: isSubmittingComment || !responseText.trim() ? "not-allowed" : "pointer",
                              opacity: isSubmittingComment || !responseText.trim() ? 0.7 : 1,
                              fontSize: "14px",
                            }}
                          >
                            {isSubmittingComment ? t("pages.ticketDetail.submitting") : t("pages.ticketDetail.submit")}
                          </button>
                        </div>
                      </form>
                    </div>
                  ...
                  <div style={{ background: "#fffdfb", borderRadius: "14px", padding: "22px", border: "1px solid #fce6d8" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        {attachments.map((file) => (
                          <button
                            ...
                            style={{
                              width: "100%", textAlign: "left", border: "1px solid #fde4d4",
                              borderRadius: "10px", background: "#fffaf7", padding: "10px 12px",
                    ...
                    <div style={{ minHeight: "280px" }}>
                      ...
                      {commentsLoading ? (
                        ...
                        <div style={{ color: "#9ca3af", fontSize: "14px" }}>{t("pages.ticketDetail.noTimeline")}</div>
                      ) : (
                        <div style={{ position: "relative", paddingLeft: "30px" }}>
                          {comments.map((comment, index) => {
                            ...
                            const dateStr = Number.isNaN(commentDate.getTime()) ? "-"
                              : commentDate.toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" });
                            const timeStr = Number.isNaN(commentDate.getTime()) ? "-"
                              : commentDate.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" });
                            const isAgentReply = ticket?.handler && comment.senderName && comment.senderName === ticket.handler;
                            return (
                              <div key={comment.id ?? index} style={{ position: "relative", marginBottom: "18px" }}>
                                {!isLast && (
                                  <div style={{ position: "absolute", left: "-20px", top: "10px", bottom: "-20px", width: "2px", background: "#FF8040" }} />
                                )}
                                <div style={{
                                  position: "absolute", left: "-30px", top: "2px",
                                  width: "22px", height: "22px", borderRadius: "50%",
                                  background: isAgentReply ? "white" : "#FF8040",
                                ...
                                <div style={{
                                  border: "1px solid #fde4d4", borderRadius: "10px", padding: "10px 12px",
                                  background: isAgentReply ? "#fffdfb" : "#FF8040",
                                }}>
                                  <div style={{ display: "flex", justifyContent: "space-between", gap: "8px", marginBottom: "4px" }}>
                                    <div style={{ fontWeight: "600", fontSize: "13px", color: isAgentReply ? "#333" : "white" }}>
                                      {comment.senderName || t("pages.ticketDetail.unknownSender")}
                                    </div>
                                    <div style={{ fontSize: "11px", color: isAgentReply ? "#9ca3af" : "rgba(255,255,255,0.75)", whiteSpace: "nowrap" }}>
                                      {dateStr} • {timeStr}
                                    </div>
                                  </div>
                                  <div style={{ fontSize: "13px", color: isAgentReply ? "#555" : "white", lineHeight: "1.5", wordBreak: "break-word" }}>
                                    {comment.message || "-"}
                                  </div>
                                </div>
                              </div>
