...
import {
  ...
  MdSupportAgent, MdPerson, MdCheckCircle, MdDelete,
  MdKeyboardArrowDown,
} from "react-icons/md";
import {
...
// Matches DB table `priority`: 1=Low, 2=Normal, 3=High, 4=Critical
    ...
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button
        ...
        style={{
          ...
          background: "transparent", border: "1px solid #fde4d4",
        ...
        onMouseEnter={(e) => { if (!disabled) e.currentTarget.style.borderColor = "#FF8040"; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#fde4d4"; }}
      >
        {display}
        {!disabled && (
          <MdKeyboardArrowDown
            size={14} color="#FF8040"
      ...
      {open && (
        <div style={{
          ...
          background: "white", border: "1px solid #fde4d4", borderRadius: "8px",
          boxShadow: "0 4px 16px rgba(0,0,0,0.14)", minWidth: "160px",
          ...
          {options.map((opt) => (
            <button
              ...
              onMouseLeave={(e) => { e.currentTarget.style.background = opt === display ? "#FFF5EF" : "transparent"; }}
            >
              {opt}
            </button>
          ))}
...
function ConfirmDialog({ message, onConfirm, onCancel }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "white", borderRadius: "14px", padding: "28px 32px", boxShadow: "0 8px 32px rgba(0,0,0,0.18)", maxWidth: "420px", width: "90%", border: "2px solid #FF8040" }}>
        <div style={{ fontSize: "16px", fontWeight: "600", color: "#333", marginBottom: "22px", lineHeight: "1.5" }}>{message}</div>
        <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{ padding: "8px 20px", borderRadius: "8px", border: "2px solid #ddd", background: "white", color: "#666", fontWeight: "600", cursor: "pointer", fontSize: "14px" }}>Cancel</button>
          <button onClick={onConfirm} style={{ padding: "8px 20px", borderRadius: "8px", border: "none", background: "#ef4444", color: "white", fontWeight: "700", cursor: "pointer", fontSize: "14px" }}>Delete</button>
        </div>
      </div>
    </div>
  );
...
export default function AdminTicketPage() {
  ...
  // CS Agents for "Assigned To" dropdown
  ...
  // Technicians for "Technician" dropdown
  ...
  // ── Dropdown options ──
  const csAgentOptions = useMemo(() => ["Unassigned", ...csAgents.map((a) => a.name).filter(Boolean)], [csAgents]);
  const technicianOptions = useMemo(() => ["Unassigned", ...technicians.map((t) => t.name).filter(Boolean)], [technicians]);

  // ── Filtering ──
  const filteredTickets = useMemo(() => {
    if (activeTab === "solved") return tickets.filter((t) => t.status === "Solved");
    return tickets.filter((t) => t.status !== "Solved");
  }, [tickets, activeTab]);

  const sortedTickets   = useMemo(() => sortTicketsFn(filteredTickets, orderBy, order), [filteredTickets, orderBy, order]);
  ...
  const activeCount = useMemo(() => tickets.filter((t) => t.status !== "Solved").length, [tickets]);
  const solvedCount = useMemo(() => tickets.filter((t) => t.status === "Solved").length, [tickets]);
  ...
  const toggleSelectAll = () => {
    if (allPageSelected) {
      setSelectedIds((prev) => { const next = new Set(prev); paginatedTickets.forEach((t) => next.delete(t.id)); return next; });
    } else {
      setSelectedIds((prev) => { const next = new Set(prev); paginatedTickets.forEach((t) => next.add(t.id)); return next; });
    }
  };
  ...
  const handleFieldUpdate = async (field, value) => {
      ...
      try {
          ...
          // Kirim null sebagai string kosong supaya backend tahu harus di-clear
  ...
  const handleTabChange    = (tab) => { setActiveTab(tab); setPage(0); setSelectedIds(new Set()); };
  const handleRequestSort  = useCallback((property) => { setOrder((prev) => (orderBy === property && prev === "asc" ? "desc" : "asc")); setOrderBy(property); setPage(0); }, [orderBy]);
  ...
  const openTicketDetail = (ticket) => { setSelectedTicket(ticket); setResponseText(""); setAttachment(null); navigateTo("detail"); };
  ...
  const handleSubmitResponse = async (e) => {
    ...
    try {
      ...
      if (message) { const result = await submitComment(message); newCommentId = result?.id ?? null; }
      if (attachment) {
      ...
      setResponseText(""); setAttachment(null);
    } catch (err) {
  ...
  const SavingBadge = ({ field }) => savingField === field
    ...
    <TableHead>
      <TableRow sx={{ borderBottom: "2px solid #f0f0f0" }}>
        <TableCell padding="checkbox" sx={{ width: "48px" }}>
          <Checkbox indeterminate={someSelected && !allPageSelected} checked={allPageSelected} onChange={toggleSelectAll}
  ...
  const renderTicketRow = (ticket) => {
    ...
    return (
      <TableRow key={ticket.id} sx={{ borderBottom: "1px solid #f0f0f0", background: isChecked ? "#FFF5EF" : "transparent", transition: "background 0.15s" }}>
        <TableCell padding="checkbox">
          <Checkbox checked={isChecked} onChange={() => toggleOne(ticket.id)} sx={{ color: "#FF8040", "&.Mui-checked": { color: "#FF8040" } }} />
        </TableCell>
        ...
        <TableCell>
          {isDispatched ? <span style={{ display: "flex", alignItems: "center", gap: "5px", color: "#333" }}><MdPerson size={16} /> {ticket.solver}</span>
            : isAssignedToSelf ? <span style={{ display: "flex", alignItems: "center", gap: "5px", color: "#FF8040" }}><MdSupportAgent size={16} /> You</span>
    ...
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div style={{ background: "white", padding: "15px 30px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 2px 5px rgba(0,0,0,0.05)", height: "70px" }}>
        {pageHistory.length > 1
          ? <button onClick={navigateBack} style={{ background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "5px", color: "#FF8040", fontWeight: "600", fontSize: "14px" }}><MdArrowBack size={20} /> Back</button>
          : <SearchBar />}
      </div>
            ...
            <div style={{ background: "white", borderRadius: "14px", boxShadow: "0 8px 28px rgba(0,0,0,0.08)", overflow: "hidden" }}>
              <div style={{ background: "linear-gradient(180deg, #FFF7F2 0%, #FFFFFF 100%)", padding: "16px 22px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                {selectedTicket.status !== "Solved" && (
                  <button onClick={handleResolveTicket} disabled={resolvingTicket} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 18px", borderRadius: "8px", border: "none", background: resolvingTicket ? "#ccc" : "#FF8040", color: "white", fontWeight: "700", fontSize: "14px", cursor: resolvingTicket ? "not-allowed" : "pointer" }}>
                    <MdCheckCircle size={18} /> {resolvingTicket ? "Resolving..." : "Mark as Resolved"}
                  </button>
                )}
                {selectedTicket.status === "Solved" && (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "24px", alignItems: "start" }}>
                  ...
                  <div style={{ padding: "12px 6px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px" }}>
                      ...
                      <div>
                        <div style={{ color: "#FF8040", fontWeight: "600", fontSize: "13px", marginBottom: "6px" }}>Created at</div>
                        <div style={{ color: "#333" }}>{formatTicketDate(selectedTicket.createdAt)}</div>
                      </div>

                      {/* Priority — Low / Normal / High / Critical sesuai DB */}
                      <div>
                        <div style={{ color: "#FF8040", fontWeight: "600", fontSize: "13px", marginBottom: "6px" }}>
                      ...
                      <div>
                        <div style={{ color: "#FF8040", fontWeight: "600", fontSize: "13px", marginBottom: "4px" }}>Customer</div>
                        <div style={{ color: "#333", display: "flex", alignItems: "center", gap: "6px" }}>
                          <MdAccountCircle size={16} color="#FF8040" />{selectedTicket.customer ?? "-"}
                        </div>
                      </div>

                      {/* Assigned To — CS Agent list dari API */}
                      <div>
                        <div style={{ color: "#FF8040", fontWeight: "600", fontSize: "13px", marginBottom: "6px" }}>
                      ...
                      <div>
                        ...
                        <InlineDropdown
                          ...
                          onSelect={(v) => handleFieldUpdate("technician", v === "Unassigned" ? null : v)}
                          disabled={selectedTicket.status === "Solved"}
                        />
                      </div>
                      ...
                      {selectedTicket.resolvedAt && (
                        <div>
                          <div style={{ color: "#FF8040", fontWeight: "600", fontSize: "13px", marginBottom: "4px" }}>Resolved at</div>
                          <div style={{ color: "#333" }}>{formatTicketDate(selectedTicket.resolvedAt)}</div>
                        </div>
                      )}
                    </div>
                    ...
                    {selectedTicket.status !== "Solved" && (
                      <div style={{ marginTop: "26px", paddingTop: "20px", borderTop: "1px solid #f1f5f9" }}>
                        ...
                        <form onSubmit={handleSubmitResponse}>
                          <textarea rows={4} placeholder="Write your response..." value={responseText} onChange={(e) => setResponseText(e.target.value)}
                          ...
                          <button type="submit" disabled={submitting || (!responseText.trim() && !attachment)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: "9px 18px", borderRadius: "8px", border: "2px solid",
                ...
                {/* Active / Solved Toggle */}
                <div style={{ display: "flex", borderRadius: "10px", overflow: "hidden", border: "2px solid #FF8040" }}>
                  {[
                    { key: "active", label: t("pages.adminTicket.active"), count: activeCount },
                    { key: "solved", label: t("pages.adminTicket.solved"), count: solvedCount },
                  ].map((tab, idx) => (
                    <button key={tab.key} onClick={() => handleTabChange(tab.key)}
                      style={{
                        padding: "9px 28px", fontWeight: "700", fontSize: "14px", cursor: "pointer", border: "none",
                        borderLeft: idx > 0 ? "2px solid #FF8040" : "none",
                    ...
                    >
                      {tab.label}
                      <span style={{ background: activeTab === tab.key ? "rgba(255,255,255,0.3)" : "#FF8040", color: "white", borderRadius: "20px", padding: "1px 8px", fontSize: "12px", fontWeight: "700", minWidth: "22px", textAlign: "center" }}>
                        {tab.count}
                      </span>
                    </button>
                  ))}
            ...
            <div style={{ background: "white", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
              {loading ? (
                ...
                <Paper elevation={0} sx={{ borderRadius: "12px", overflow: "hidden" }}>
                  ...
                  <TablePagination
                    component="div" count={sortedTickets.length} page={safePage}
