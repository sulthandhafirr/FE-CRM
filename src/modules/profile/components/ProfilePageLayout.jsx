...
// ---------------------------------------------------------------------------
// Validasi password kuat (khusus ganti password di profile)
// ---------------------------------------------------------------------------
...
const getPasswordChecks = (password, userName, userEmail) => {
...
const passwordErrorKey = (password, userName, userEmail) => {
  const failed = getPasswordChecks(password, userName, userEmail).find((c) => !c.ok);
  if (!failed) return null;
  if (failed.key === "length") return "passwordTooShort";
  if (failed.key === "upper" || failed.key === "lower" ||
      failed.key === "number" || failed.key === "symbol") return "passwordWeak";
  if (failed.key === "common" || failed.key === "sequence") return "passwordCommon";
...
export default function ProfilePageLayout({ tier = null, showPosition = false }) {
  ...
  const handleChangePassword = async (e) => {
    ...
    const pwdErrorKey = passwordErrorKey(
      passwordValues.newPassword,
      authName,
      user?.email
    );
    if (pwdErrorKey) {
      showMessage("error", t(`pages.profile.${pwdErrorKey}`), "security");
      return;
    }
  ...
  const renderPasswordField = (field, label, autoComplete) => (
    <div style={{ marginBottom: 16 }}>
      ...
      {/* Checklist kekuatan password saat mengetik password baru */}
      {field === "newPassword" && passwordValues.newPassword && (
        <div
          style={{
            marginTop: 8,
            display: "flex",
