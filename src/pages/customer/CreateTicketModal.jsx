...
export default function CreateTicketModal({ open, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        // Create The ticket and get the ticket id
      const { data: ticketData } = await api.post("/api/tickets", {
      ...
      // If there are attachments, get the upload URLs and upload each file to Supabase Storage
      if (formData.attachments.length > 0) {
        const { data: uploadUrls } = await api.post("/api/tickets/upload-url", {
          ticketId,
        ...
        // Upload each file to Supabase Storage using the signed URLs
        for (let i = 0; i < formData.attachments.length; i++) {
          ...
          const { error } = await supabase.storage
            ...
            .uploadToSignedUrl(urlInfo.filePath, urlInfo.token, file, {
              contentType: file.type,
            });
        ...
        // After all files are uploaded, save the attachment info to the database
        await api.post(
      ...
      // Invalidate the "my-tickets" query to refresh the ticket list
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Stack direction="row" justifyContent="flex-end" spacing={1.5}>
                ...
                <Button
                  ...
                  sx={{
                    borderRadius: "10px",
                    backgroundColor: isSubmitting ? "#ffb899" : "#FF8040",
                    px: 4,
                    py: 1,
                    fontWeight: 700,
                    boxShadow: "0 6px 16px rgba(255,128,64,0.35)",
                  }}
                >
                  Submit
                </Button>
