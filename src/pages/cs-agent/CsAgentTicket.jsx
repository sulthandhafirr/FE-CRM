...
export default function CsAgentTicket() {
  ...
  const handleSubmitResponse = async (e) => {
    ...
    try {
      ...
      if (selectedTicket.status === "Waiting") {
        await supabase.from("ticket").update({ status: "Progress" }).eq("id", selectedTicket.id);
  ...
  const handleTakeAction = async (ticket) => {
    try {
      const { error } = await supabase.from("ticket")
        .update({ solver: "You", status: "Progress" }).eq("id", ticket.id);
      if (error) throw error;
      fetchTickets();
    } catch (err) {
  ...
  const handleDispatch = async (ticket) => {
    ...
    try {
      const { error } = await supabase.from("ticket")
        .update({ solver: techName, status: "Progress" }).eq("id", ticket.id);
      if (error) throw error;
      fetchTickets();
    } catch (err) {
                    ...
                    return (
                      <div key={event.id || idx} style={{ position: "relative", marginBottom: "30px" }}>
                        <div style={{
                          ...
                          {isAgent ? <MdSupportAgent size={16} color="#FF8040" /> : <MdAccountCircle size={16} color="#FF8040" />}
                        </div>
                              ...
                              {isAssignedToSelf ? (
                                <button
                                  ...
                                  style={{
                                    ...
                                    border: "2px solid #FF8040", background: "white",
                                    color: "#FF8040", fontWeight: "600", cursor: "pointer", fontSize: "13px",
                                  }}
                                >
                                  Follow up
                                </button>
